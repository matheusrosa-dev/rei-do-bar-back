...
**Exceptions**: the admin categories and settings listings are not paginated — they return a flat array with no `meta`. No admin dashboard endpoint is a listing: each carries neither `meta` nor a top-level array, and none accepts a pagination param. Their shapes differ. The **delivery-person performance** reading returns an object pairing aggregate counters with the per-row breakdown they sum to. The **revenue** reading also pairs an aggregate half with a breakdown half, but the breakdown is a **time series** rather than a roster of entities. The performance reading's aggregate half also carries `averageDeliveryMinutes` and `averageCancellationAfterShippingMinutes`, each the mean span from the moment an order left for delivery to the moment it closed, rounded once to the nearest whole minute. They are aggregate-only, with no per-row counterpart, and they answer `null` — never `0` — when the period holds no order to measure, so a `0` means a real average under half a minute rather than an empty sample. They cover only the orders carrying a dispatch timestamp, which is a strict subset of the ones counted beside them: an order cancelled before it ever left, and any order dispatched before that timestamp was recorded at all, counts without being averaged. Its breakdown half covers only the delivery persons with at least one closed order in the requested period — a delivery person with nothing closed is absent rather than present with zeros. The revenue reading reports the summed order totals — delivery fee included, already net of any coupon — beside the summed coupon discount, both in cents; a promotional product price is absorbed into the revenue figure and is not reported as a discount, so the discount field answers coupons only. It also reports, as `couponDiscountPercentage`, what share of the gross that discount was — the coupon discount over the revenue **plus** that same discount, since the revenue is already net of it — as a number with two decimals, without a `%` sign, bounded by 100. Over a period with nothing delivered the sums answer `0`, not `null` — a sum over no rows has a meaningful identity where a mean does not — but the share answers `null`, since a ratio over no gross has nothing to measure and `0` would read as a period that sold and gave nothing away. Its counter of delivered orders shares a name with the performance reading's, but not a scope: this one counts every delivered order, that one only the orders that reached a delivery person, so for the same period the two legitimately differ. Its breakdown half repeats those same four figures over successive periods, in ascending order, each point carrying a display-ready pt-BR `label` and nothing else identifying it — the array order is the whole ordering contract, and the label's format is part of the contract rather than presentation. The period each point spans is chosen by the server, not the caller, from the span in calendar days: one per **hour** when both bounds fall on the same day, one per **day** up to 62 days, one per **month** beyond that or whenever a bound is missing — so the label reads `14:00`, `26/08` or `Agosto/2026` respectively. Points are bucketed by the `America/Sao_Paulo` calendar — the one place either reading consults a timezone, and deliberately not extended to the bounds. **A caller that sends bare dates therefore gets labels shifted by a day** (a bare `2026-08-01`–`2026-08-31` is read as UTC midnight and labelled `31/07`…`30/08`), so sending the offset is required to get the axis the caller means. When both bounds are sent the series is also capped at 600 points, truncated rather than rejected past it — and since buckets run oldest-first, truncation drops the most recent periods. An inverted range yields an empty series. When both bounds are sent, every period in the interval appears, including the ones with nothing delivered, which report zeros; otherwise only the periods that had a delivery appear. The aggregate half equals the sum of the series in every case but one: a series truncated by the cap drops periods whose orders still count in the aggregates. That holds for the three summable figures; the discount share is recomputed from each period's own gross, so it is neither the sum of the points nor their average, and a period with no gross carries `null` while the ones around it carry numbers. The revenue reading only counts delivered orders that carry a delivery timestamp, with or without a range, so both halves always describe the same rows. Both readings take the same query params, filters only, with no pagination param to go alongside them: `startDate` and `endDate`, each optional and independent, and nothing else. Both are parsed as instants and applied verbatim as inclusive bounds — the server does no timezone adjustment and no day-boundary snapping, so a caller that means a civil day sends the offset itself. Omitting both is what yields lifetime figures. A range is matched against the timestamp that closed the order, which older rows do not carry, so those rows count in the lifetime figures and are absent from every range — filtered figures are not comparable to unfiltered ones on either reading. The products, customers, and delivery persons listings support `simple=true` for the same flat, unpaginated shape.
