...
Five read-only endpoints, none paginated. All take the same two optional, independent params —
...
| `GET /admin/dashboard/delivery-persons` | `{ deliveryPersons: [...] }` | Per row: `name` (the delivery person's, and the row's only identifier — no id is returned), `deliveredOrdersCount` (delivered orders **not** flagged volunteer), `volunteeredDeliveriesCount` (delivered orders flagged volunteer — disjoint from `deliveredOrdersCount`; total delivered is the two summed), `cancelledOrdersCount` (all cancelled, volunteer or not), `deliveryFeeTotal` (cents — **delivered, non-volunteer** orders only), `deliveryPersonBonusTotal` (cents — bonus over **closed, non-volunteer** assigned orders; volunteer orders contribute nothing — so this differs from the summary's same-named field, which still counts volunteer rows), `payoutTotal` (cents — `deliveryFeeTotal + deliveryPersonBonusTotal`), `volunteeredSavingsTotal` (cents — what the volunteer orders would have cost: their `deliveryPersonBonus` over every closed one plus `deliveryFee` over the delivered ones) |
| `GET /admin/dashboard/rankings` | `{ products: [...], coupons: [...] }` | Two ranked lists, **at most 5 rows each**, over delivered orders — a shorter list means the range held fewer keys, and an empty range answers two empty arrays (never zeroed rows). Per product row: `name` and `imageUrl` (the product's current values, `name` is the row's only identifier — no id is returned), `soldQuantity` (units sold), `ordersCount` (orders the product appeared in), ordered by `soldQuantity` desc. Per coupon row: `code` (the code frozen on the order — it may be the settings-driven welcome coupon, which has no entry in the admin coupons registry), `ordersCount` (orders that carried the code), `discountTotal` (cents), ordered by `ordersCount` desc. Both lists are a **head, not a partition**: their rows never add up to a period total |
...
| `GET /admin/dashboard/summary` | Flat object, 17 fields at the top level of `data` | `deliveredOrdersCount`, `failedDeliveriesCount`, `averageOrderValue`, `highestOrderValue`, `redeemedCouponOrdersCount`, `firstDeliveredOrdersCount`, `newCustomersCount`, `averageDeliveryMinutes`, `revenue`, `deliveryFeeTotal` (cents — the delivery-fee slice of delivered, stamped, **non-volunteer** orders; volunteer deliveries are excluded, so this is no longer the whole fee slice inside `revenue`), `deliveryPersonBonusTotal` (cents — the bonus paid to entregadores over **closed, non-volunteer** assigned orders, cancelled ones included, so its universe is wider than every other money field here; unlike `deliveryFeeTotal` it is **not** inside `revenue`), `volunteeredSavingsTotal` (cents — what the volunteer orders would have cost: their bonus over every closed one plus the delivery fee over the delivered, stamped ones), `restockCost`, `profit`, `profitPercentage`, `couponDiscount`, `couponDiscountPercentage` |
