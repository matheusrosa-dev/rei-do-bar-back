...
| `GET /admin/dashboard/delivery-persons` | `{ deliveryPersons: [...] }` | Per row: `name` (the delivery person's, and the row's only identifier — no id is returned), `deliveredOrdersCount`, `cancelledOrdersCount`, `deliveryFeeTotal` (cents — **delivered** orders only, cancelled ones excluded), `deliveryPersonBonusTotal` (cents — bonus over **closed** assigned orders, cancelled ones included), `payoutTotal` (cents — `deliveryFeeTotal + deliveryPersonBonusTotal`) |
...
| `GET /admin/dashboard/summary` | Flat object, 16 fields at the top level of `data` | `deliveredOrdersCount`, `failedDeliveriesCount`, `averageOrderValue`, `highestOrderValue`, `redeemedCouponOrdersCount`, `firstDeliveredOrdersCount`, `newCustomersCount`, `averageDeliveryMinutes`, `revenue`, `deliveryFeeTotal` (cents — the delivery-fee slice of delivered, stamped orders, already inside `revenue`), `deliveryPersonBonusTotal` (cents — the bonus paid to entregadores over **closed** assigned orders, cancelled ones included, so its universe is wider than every other money field here; unlike `deliveryFeeTotal` it is **not** inside `revenue`), `restockCost`, `profit`, `profitPercentage`, `couponDiscount`, `couponDiscountPercentage` |
