...
You are running as a **fresh, independent subagent** with no access to the implementing conversation. Review **only** what is passed via `$ARGUMENTS`; do not infer, expand, or pull in unrelated files. **Never edit any file — report only.**

Changed files and short description of what was implemented: $ARGUMENTS

## Procedure

1. **Receive the changed files** and the short description of what was implemented.
2. **Read directory conventions** — for every directory involved, read its `AGENTS.md`; always read the root `AGENTS.md` first, and `.opencode/references/api-contract.md` for the response/error/pagination contract.
