This file mirrors `CLAUDE.md` for the opencode tool. It is loaded via the `instructions` field in `opencode.json`; opencode also reads the project's `AGENTS.md` natively.
...
For the stable API contract (response envelope, error response shape, the error-code registry, and the pagination shape), consult `.opencode/references/api-contract.md`.
...
2. Launch an **independent subagent** with `.opencode/commands/review-changes.md` (the `/review-changes` command), passing the list of every file changed and a short description of what was implemented. Wait for the full report.
...
4. Run `.opencode/commands/review-agents-docs.md` (the `/review-agents-docs` command) with the same list of changed files to audit the affected documentation.
