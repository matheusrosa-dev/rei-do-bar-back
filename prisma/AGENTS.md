...
| Snapshots | Order rows denormalize into their own columns the data a later change must not rewrite. Two kinds: purchase-time data (item details, delivery address, applied coupon code and discount) frozen once at creation; and assignment data — who fulfils the order and what they are owed — written whenever the delivery person is attached or swapped, each write re-snapshotting the party bound at that moment (a reassignment therefore reprices and re-copies their flags), but no unrelated write ever touching it. A snapshot may coexist with a nullable live reference to the source row (nulled on deletion) when the reference is needed operationally |
