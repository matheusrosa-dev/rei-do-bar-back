# AGENTS.md — src/delivery-persons/

## What belongs here

The delivery-person-facing surface of the API — what an entregador consumes from the delivery app while working. It is a **container module only**: it owns no controller or service of its own, just the sub-modules it registers (`auth/` and `orders/`), mirroring how `src/admin/` is organized.

## What does NOT belong here

- Delivery-person CRUD (create, update, activate, delete), **password assignment**, and **access revocation** → `src/admin/delivery-persons/`.
- Order status transitions and delivery-person assignment → `src/admin/orders/`.
- Customer order placement, listing, and cancellation → `src/orders/`.
- Customer authentication (OTP + JWT) → `src/auth/`. The two flows share nothing but the shared helpers; do not merge them.
...
A delivery person with **no password set cannot log in at all**, and neither can an inactive one. Deactivating someone both deletes their session and makes the guard reject any token that survives, so it remains a complete cut-off — but the admin can now also revoke access on its own, without touching the active flag.
