# AGENTS.md — src/delivery-persons/orders/
...
- Assigning or reassigning a delivery person to an order → `src/admin/orders/` (assignment happens on the transition to `SHIPPED`).
- Moving an order to `DELIVERED` → `src/admin/orders/`, through the status state machine.
