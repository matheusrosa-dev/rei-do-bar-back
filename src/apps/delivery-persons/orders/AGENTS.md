...
- **Order**: oldest first, by **dispatch** time (`shippedAt asc`, tied on `orderNumber`) — the list is a delivery queue, not an archive, so the opposite of the customer-facing listing. Dispatch time, not `createdAt`: the queue's position comes from when the admin sent each order out, not from when the customer placed it, and the admin board sorts its shipped column the same way — two surfaces over the same set of orders must not disagree on queue position. The nulls sort **first**: the column was added without a backfill, so an order dispatched before that migration carries no stamp and is by definition the oldest thing in the queue, while Postgres would sort it last under `ASC`.
...
Read the DTO as two groups. The **order group** is a deliberately narrow subset of the customer order response — only what someone carrying the order to the door needs: the `id` to confirm the delivery against, the `orderNumber` to call it out, the snapshotted `address` to deliver to, the `total` and `paymentType` to collect the right amount, and the items (id, name, image, quantity, unit price) to check the bag against — read oldest-first and tied on the id, so the same order lists its lines the same way every time the entregador opens it. Everything else the service computes is dropped at serialization: `status` (invariant on the listing), the row's lifecycle timestamps, and the whole breakdown that only matters for billing (`deliveryFee`, `productsTotal`, `productsDiscount`, `couponDiscount`, `couponCode`, `statusReason`, `compareAtPrice` on items).
