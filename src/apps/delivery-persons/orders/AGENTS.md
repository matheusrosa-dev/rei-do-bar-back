...
Read the DTO as two groups. The **order group** is a deliberately narrow subset of the customer order response — only what someone carrying the order to the door needs: the `id` to confirm the delivery against, the `orderNumber` to call it out, the snapshotted `address` to deliver to, the `total` and `paymentType` to collect the right amount, and the items (id, name, image, quantity, unit price) to check the bag against — read oldest-first and tied on the id, so the same order lists its lines the same way every time the entregador opens it. Everything else the service computes is dropped at serialization: `status` (invariant on the listing), `createdAt`, and the whole breakdown that only matters for billing (`deliveryFee`, `productsTotal`, `productsDiscount`, `couponDiscount`, `couponCode`, `statusReason`, `compareAtPrice` on items).
