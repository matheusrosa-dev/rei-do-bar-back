...
- **Endpoints**: `GET delivery-persons/orders`, `GET delivery-persons/orders/delivered-count`, and `PATCH delivery-persons/orders/:orderId/deliver`. The controller now mixes **static and param segments**, so declaration order matters: a static route must be declared before any same-verb param route that could match it, or it is shadowed silently — nothing errors, the wrong handler just runs. The delivery person always comes from the **access token**, read through `@CurrentDeliveryPerson()` — the only id ever accepted from the client is the **order** id, never a delivery-person id. Every controller in this module states the `accessToken` level of the audience composite at class level, which puts the app credential in front of the token guard; a controller here never sits at a lower level. See the parent `AGENTS.md`.
...
- **Order**: oldest first, by **dispatch** time (`shippedAt asc`, tied on `orderNumber`, nulls first) — a delivery queue, not an archive, so the opposite of the customer-facing listing. Dispatch time, not `createdAt`: queue position comes from when the admin sent the order out, matching how the admin board sorts its own shipped column — two surfaces over the same orders must not disagree on queue position. Nulls sort first because the column has no backfill: an order dispatched before that migration carries no stamp and is by definition the oldest in the queue, where Postgres would otherwise sort it last under `ASC`.
...
- **The window comes from the shared recent-orders helper**, not from a constant of this module's own and not from configuration. The admin management board and the admin delivery-person listing read the same helper — and the listing's shift figure is *this very count*, shown to the operator instead of to the entregador, so a local copy here would let the two surfaces report different numbers for the same shift with nothing failing. `@shared/helpers/date.ts` stays out of it: that file is for timezone-aware calendar boundaries, and a rolling window is plain arithmetic with no zone.
...
Read the DTO as two groups. The **order group** is a deliberately narrow subset of the customer order response — only what someone carrying the order to the door needs: the `id` to confirm the delivery against, the `orderNumber` to call it out, the `customerName` to confirm who is answering the door, the snapshotted `address` to deliver to, the `total` and `paymentType` to collect the right amount, and the items (id, name, image, quantity, unit price) to check the bag against — read oldest-first and tied on the id, so the same order lists its lines the same way every time the entregador opens it. `customerName` comes from a `select`-scoped `customer` include, never the full relation, and can be `null` — `Customer.name` is optional. Everything else the service computes is dropped at serialization: `status` (invariant on the listing), the row's lifecycle timestamps, and the whole breakdown that only matters for billing (`deliveryFee`, `productsTotal`, `productsDiscount`, `couponDiscount`, `couponCode`, `statusReason`, `compareAtPrice` on items).
...
| Time of delivery comes from `deliveredAt`, time of dispatch from `shippedAt` | Never infer either from `updatedAt`, which a later write silently overwrites |
| The queue is ordered by dispatch time | `shippedAt asc` with nulls first, tied on `orderNumber` — the same order the admin board gives its shipped column, so the operator and the entregador see one queue |
