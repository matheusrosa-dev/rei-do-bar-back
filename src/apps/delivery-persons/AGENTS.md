...
A delivery person with **no password set cannot log in at all**, and neither can an inactive one. Deactivating someone both deletes their session and makes the guard reject any token that survives, so it remains a complete cut-off — but the admin can now also revoke access on its own, without touching the active flag. Revocation is the harder of the two on the credential: it clears the stored password hash along with the session, so a revoked delivery person stays active yet unable to log in until the admin assigns a new password (see `src/apps/admin/delivery-persons/AGENTS.md`).
