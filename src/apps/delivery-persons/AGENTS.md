...
**Every route on this surface also carries the app credential**, a fixed HTTP Basic pair sent as `x-delivery-person-authorization: Basic <base64(username:password)>` and validated against the `deliveryPerson` config namespace. It proves the call came from the delivery *app* and says nothing about *who* is calling, so it is additive to whatever token the route requires — an authenticated call carries it **and** `Authorization: Bearer <token>` at once. It rides its own header precisely because `Authorization` is already spent on the opaque bearer token, and it applies to `login` too: that route mints the first token pair, but it is no longer credential-less.

Every controller states its level through the delivery-person auth composite, which puts the basic-auth guard first at every level. Controllers outside `auth/` sit at `accessToken` at class level; inside `auth/` the levels differ per handler — `login` at `basic`, `refresh` at `refreshToken`. The delivery app sends no `x-device-id`, and no guard on this surface asks for one. See `src/shared/guards/AGENTS.md`.
