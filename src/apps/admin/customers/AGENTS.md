...
- **Listing**: paginated, filtered, and searchable by name / phone / id; soft-deleted customers are excluded. Items are enriched with the customer's main address and three order counts (all, cancelled, delivered). Sorting by the **total** order count is done at the DB level (`orderBy` on the relation `_count`); sorting by the **delivered** count cannot be expressed that way — it is a filtered count — so it falls back to the in-memory two-step fetch: ids plus their counts, sorted and sliced in memory, then the page refetched by id and reordered to match. That first query carries its own `orderBy` even though the sort happens afterwards, and it is load-bearing — see the conventions table. Every sort branch of this listing, database-level ones included, ends on a unique tiebreaker for the same reason. An opt-in flag returns a flat, unpaginated list — every **active**, non-soft-deleted customer ordered alphabetically by name and tied on the id, without the enrichment — intended for pickers/selects rather than the paginated page object.
...
| Every sort ends on a unique tiebreaker | Ties under `skip`/`take` are sliced in whatever order Postgres happens to return, which shifts as rows are updated, so a customer surfaces on two pages or on none. The tie sets here are large — most rows sit on zero orders — and `createdAt` is not unique, so each branch appends the id. On the in-memory path the `orderBy` sits on the **ids** query and looks redundant next to the sort that follows; it is not, because that sort is stable and merely preserves whatever order arrived |
