# AGENTS.md — src/apps/admin/coupons/
...
- Coupon redemption rules (availability, discount calculation, usage limits) → the shared coupons module (`src/apps/store/coupons/`); assigning/removing a coupon on a cart → the cart module; revalidation and usage recording at order placement → the orders module.
- **The welcome coupon.** It is not a `Coupon` row and is not manageable here. It lives as a discount amount in cents in the `WELCOME_COUPON` runtime setting (edited through `src/apps/admin/settings/`) and is evaluated by `src/apps/store/coupons/`. Creating a real coupon whose code happens to be the welcome code does **not** make it the welcome coupon.

---

## Core Patterns

- **Listing**: paginated, filtered by `isActive` / `hasStarted` / `isFinished`, with free-text search on the **`code` only** (not the id), defaulting to `startsAt` ascending. Each item is enriched with `usageCount`, `assignedCustomers`, `hasStarted`, and `isFinished`. `usageCount` is sorted at the DB level via `orderBy: { usages: { _count } }`; every ordering ends on the id as a unique tiebreaker, the `startsAt` default included, `usageCount` most of all — unredeemed coupons all sit on zero, so the tie set is close to the whole table; determining which coupons have exhausted their usage limit needs a comparison between two columns, so it goes through a **raw SQL query** that returns the ids of the limit-reached coupons.
...
| Welcome coupon is not a coupon | It is a setting, not a row here — see `src/apps/admin/settings/` and `src/apps/store/coupons/` |
