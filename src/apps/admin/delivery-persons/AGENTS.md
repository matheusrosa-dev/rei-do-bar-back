...
- **Listing**: paginated, filterable by `isActive`, searchable by `name`/`phone`/`cpf` (case-insensitive `contains`). Fixed newest-first, no sort param — `ordersCount` (delivered only, a filtered relation count Prisma cannot `orderBy`) is why: sorting on it needs the two-step in-memory path the customers listing uses, not added lightly. Ends on `id` as tiebreaker. Each row adds `ordersCount`, `recentDeliveredCount`, `hasAccess`. The opt-in flat variant (`simple=true`) is hard-filtered to active rows with a `hashedPassword` (not the `hasAccess` disjunction — a live session with no password is a revoked credential, and the picker should stop offering them), sorted alphabetically, meant for assignment pickers.
- **`recentDeliveredCount`** is the shift figure beside the lifetime `ordersCount`: closed (`DELIVERED`, `deliveredAt` inside the window) in the **last 10 hours** — the same metric `GET delivery-persons/orders/delivered-count` reports to the entregador themselves, read off the same shared window helper so the two surfaces can never disagree. It cannot be a second `_count` (Prisma allows one filter per relation there), so it is a separate `order.groupBy` scoped to the fetched page's ids, run after the page query, evaluated against the same `now` as `hasAccess`. It closes on `deliveredAt`, never `updatedAt` (which a later reassignment would bump) — meaning the `20260818111540_add_order_delivered_at` backfill's 10-hour cutoff still applies: rows delivered inside that boundary at migration time hold a null stamp and do not count.
...
- **Read shape**: every method returning a delivery person (listing, single read, create, update, password write, toggles) uses the same `helpers.ts` mapper — the four flat `address*` columns nest into `address`. `ordersCount` is on listing and single read only (both via the shared delivered-orders selector); `hasAccess` and `recentDeliveredCount` are listing-only (the single read has no session `select` to compute the first from, and adding it is a query change, not a mapper change).
- **Creation**: starts **active** — no client-facing listing means there is nothing to protect by defaulting inactive.
- **Update**: `name`, `phone`, `cpf`, address. Phone/CPF collisions both map to `DELIVERY_PERSON_ALREADY_EXISTS`.
...
- **`hashedPassword` is returned on every admin response of this resource, deliberately** — the admin surface is operator-only behind Basic Auth, and narrowing the projection at every call site was judged not worth it. The exposure is bounded (bcrypt cost 10 — not directly usable, only crackable offline if a response leaks); the mitigation is password policy and admin-credential hygiene, not the response shape. If that trade is ever revisited, narrow it in `mapDeliveryPerson` (one place), not per query.
...
| `address` | Nested object — `street` (1–100), `number` (1–10), `neighborhood` (1–100), `zipCode` (8 digits) — same rules as a customer's address |
...
| One shape across the resource | `helpers.ts` nests the address everywhere; `ordersCount` on listing + single read only, via the shared delivered-orders selector |
...
| `recentDeliveredCount` is a `groupBy`, not a `_count` | `ordersCount` already spends the relation's one filter slot. Scoped to the page's ids, run after the page query, on the same `now` as `hasAccess` |
| The shift window is shared, never a local copy | Length + window-start live in the shared recent-orders helper, read here and by the delivery app's own count — a local copy would let the two surfaces silently disagree about the same shift |
