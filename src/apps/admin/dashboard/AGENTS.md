...
| `redeemedCouponOrdersCount` | summary | delivered orders carrying a stamp, `couponDiscount > 0` | Counts **orders**, not distinct coupons (two orders on the same coupon count twice). Divisor for the per-redemption average: `couponDiscount / redeemedCouponOrdersCount` |
