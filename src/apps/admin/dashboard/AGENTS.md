...
the summary's two entregador-money fields, `deliveryFeeTotal` and `deliveryPersonBonusTotal`, are
the deliberate exceptions: period aggregates with no matching series line. Both decompose per
order and *could* be plotted; they are not, because the order series answers what the store
**earned** per bucket and these two are what it **pays out**. A third payout figure belongs beside
them, not on the series. The performance breakdown carries the same two names **per delivery
person** (not a period aggregate) — but on their own universes: `deliveryFeeTotal` there is
delivered-only, `deliveryPersonBonusTotal` spans both terminal statuses, and the bonus sum
reconciles with the summary's while the fee sum does not. It also adds the two into a per-row
`payoutTotal`, which the summary has no counterpart for. No reading other than the summary
carries a `totals`/aggregate half.
...
| Performance | `GET /admin/dashboard/delivery-persons` | `{ deliveryPersons: [...] }` — a breakdown and nothing else | Orders **assigned to a delivery person and closed** (delivered or cancelled) — the two counts and `deliveryPersonBonusTotal` span both terminal statuses; `deliveryFeeTotal` alone narrows to the **delivered** slice; `payoutTotal` is the two money fields added per row |
...
| Summary | `GET /admin/dashboard/summary` | A flat object, 16 fields, no envelope | Every **closed** order (delivered or cancelled), assigned or not — the widest of the `order`-based readings — though its cancelled half surfaces only through `failedDeliveriesCount` and `deliveryPersonBonusTotal` (assigned rows both): no field here counts cancelled orders as a whole |
...
| `deliveryFeeTotal` | performance (per person), summary (period) | performance: assigned orders that were **delivered** — a cancelled order's fee is excluded (its `_sum` rides the `DELIVERED` group alone) · summary: delivered orders carrying a stamp, assigned or not — same universe as `revenue` | Cents. **Two different universes under one name.** On performance it is the delivery fee for the orders this person actually delivered — a cancelled order contributes nothing here, so it is near what they are owed but not exact: it reconciles with **nothing** on the summary, whose figure counts unassigned orders too and (unranged) requires a stamp the roster's delivered branch does not, so `Σ deliveryPersons[].deliveryFeeTotal` matches no field there. On summary it is the delivery-fee slice already embedded in `revenue` (which nets the coupon out on top of it), broken out on its own — and also subtracted back out of `profit`, since the fee is handed to the entregador rather than kept; it rides the same `sumRevenue` reducer as `revenue`/`couponDiscount`, but unlike them has **no** series counterpart to sum up from — the order series does not report it |
| `deliveryPersonBonusTotal` | performance (per person), summary (period) | assigned orders that closed, delivered **and** cancelled alike (`buildAssignedClosedFilter`) — the **same predicate** on both readings: the roster's `groupBy` `_sum` per person, the summary's own `aggregate` `_sum` period-wide | Cents. The bonus the store pays the entregador **on top of** the delivery fee: a cancelled order's bonus is already handed over, so it counts on both readings. Because the two reads share the predicate exactly, `Σ deliveryPersons[].deliveryPersonBonusTotal` **does** reconcile with the summary's figure for the same range (two queries over one predicate, like `failedDeliveriesCount` vs `Σ cancelledOrdersCount`). On the summary it is that reading's one genuine payout figure and **three things set it apart from every other money field there** — it spans both terminal statuses instead of delivered-only; it is money that never entered `revenue` (so `profit` subtracts it as a pure outflow, where `deliveryFeeTotal` is subtracted only to return an embedded slice to zero); and it does **not** ride the `sumRevenue` reducer, earning its own `aggregate` `_sum` because that reducer's rows are delivered-only. On performance it is wider than that row's `deliveryFeeTotal` (delivered-only): the two coincide per order only on delivered rows, and a cancelled row carries a bonus with no matching fee. It has no series counterpart on either reading. Its per-order value is frozen at write time, so the total is never reproducible as `setting × any counter here` and a setting change never rewrites history |
| `payoutTotal` | performance (per person) | not a universe of its own — `deliveryFeeTotal + deliveryPersonBonusTotal` for the same row | Cents. A derived per-row sum, computed once from the row's two finished money fields, never accumulated per order and with no reducer of its own — so it carries both parents' asymmetry at once (fee is delivered-only, bonus spans both terminal statuses). It is what the entregador took home for the window: fees for what they delivered plus bonus for everything assigned that closed. Reconciles with **no** summary field — `deliveryFeeTotal` there is a different universe, so only the bonus half of this sum lines up across readings |
...
- **⚠️ `deliveryPersonBonusTotal` is a payout figure, but three things still hold it below the
  cash actually handed over** (on both the summary and the per-person performance row — same
  column, same caveats). The order column (`Int @default(0)`) is written **only** on the
  assignment transition, snapshotting the `DELIVERY_PERSON_BONUS` setting as it stood at that
  instant. So (a) every order shipped before the column existed carries a defaulted `0` and drags
  lifetime and back-dated ranges down — a value-level cousin of the backfill caveat above,
  invisible to it because no timestamp is missing; (b) the settings read behind the snapshot drops
  inactive rows and coerces a missing key to `0`, so **deactivating** the setting — not only
  zeroing it — silently stamps `0` on every order shipped while it is off; and (c) the range still
  filters on the **closing** stamp, so a bonus already committed on an order still in transit at
  the end of the window lands in a later period, never this one. Consequence for the "no field
  returns `null`" rule below: a `0` here has a third reading beyond empty-sample and real-zero —
  the setting was off, or predates the column, for those orders.
...
- **A cancelled order can legitimately carry a delivery-person assignment** (`SHIPPED →
  ...
  only counts: the cancelled order's `deliveryPersonBonus` lands in that person's
  `deliveryPersonBonusTotal` (its `deliveryFeeTotal` is delivered-only, so the fee does **not**
  follow), and a retroactive reassignment shifts that bonus from one person to another.
