...
| `deliveryFeeTotal` | performance (per person) | the roster's own universe — assigned orders, delivered **and** cancelled alike | Cents. Not an earnings or payout figure: a cancelled order's fee counts the same as a delivered one, so read it as "fee attached to the orders that reached this person", never as what they are owed. Reconciles with **nothing** on the summary — that reading's `revenue` embeds the fee of delivered orders only (and, unranged, of stamp-optional ones) and of unassigned orders too, so `Σ deliveryPersons[].deliveryFeeTotal` matches no field there |
