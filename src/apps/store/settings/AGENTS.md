# AGENTS.md — src/apps/store/settings/
...
| Rule | Detail |
|---|---|
| Session-less read | The controller carries `@StoreAuth("basic")` — the app credential only, no `x-device-id` and no JWT. The active settings are the same for every visitor, and the client needs them before it has a session |
