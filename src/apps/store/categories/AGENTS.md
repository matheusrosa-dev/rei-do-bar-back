...
The controller carries **`@StoreAuth("basic")`** — the store app credential and nothing else, so the listing requires neither an `x-device-id` header nor a JWT. That is a deliberate choice, not an omission: the category list is the same for every visitor and reveals nothing session-specific, so it needs no session — but, like every store route, it still needs the app credential. It returns only active categories, ordered by an explicit sort field.
