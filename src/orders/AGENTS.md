...
Each order carries DB fields plus computed totals and a nested item collection. The money shape **mirrors the cart's** (see `src/cart/AGENTS.md`), with one structural difference: everything is derived from the **item snapshots**, never from the live product row — a later price or sale change must not reprice a past order.
