...
**Welcome coupon**: when the cart carries no real coupon, the customer's welcome-coupon discount (see `src/coupons/AGENTS.md`) is computed — eligibility query first, then the discount — and folded into `discount` before the minimum-order check. Because it isn't backed by a `Coupon` row, the order snapshots `couponId: null` and `couponCode: WELCOME_COUPON_CODE`, and only when the computed discount is actually **greater than zero**; an eligible customer whose subtotal never met the coupon's minimum gets no coupon code on the order at all. (The cart is looser here — it reports the code on eligibility alone; see `src/cart/AGENTS.md`.) No `CouponUsage` row is created and cancellation has nothing to revert — eligibility is re-derived from order history, so cancelling the order already restores it.
