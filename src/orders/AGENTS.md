...
**Welcome coupon**: when the cart carries no real coupon, the customer's welcome-coupon discount (see `src/coupons/AGENTS.md`) is computed — eligibility query first, then the discount — and folded into `discount` before the minimum-order check. Because it isn't backed by a `Coupon` row, the order snapshots `couponId: null` and `couponCode: WELCOME_COUPON_CODE` whenever the customer is **eligible** (`isWelcomeCoupon`), the same condition the cart uses — not gated on the computed discount being non-zero. An eligible customer whose subtotal never met the coupon's minimum still gets the coupon code snapshotted on the order with `discount: 0`, mirroring what they saw in the cart. No `CouponUsage` row is created and cancellation has nothing to revert — eligibility is re-derived from order history, so cancelling the order already restores it.
