...
- **Immutable code**: the coupon `code` is unique and is set **only at creation** — the update DTO has no `code` field, so a coupon's code can never change. A duplicate code on create is translated from the Prisma unique-constraint error into a conflict `AppException`.
...
- **Monetary values**: `discountValue` (for fixed discounts) and `minOrderValue` are integers in cents; `discountValue` for percentage discounts is a whole-number percentage. Note this is a **convention, not a validated constraint** — the DTO only requires a non-negative integer, so nothing currently stops a percentage above 100.
...
| Unique, immutable code | Codes are set at creation only; duplicates raise a conflict error |
