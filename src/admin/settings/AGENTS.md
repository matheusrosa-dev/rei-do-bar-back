...
- `src/coupons/` reads it as a discount amount in cents to compute a first-order discount; the update route now rejects a non-integer value up front, so it can no longer reach that consumer as `NaN`.
