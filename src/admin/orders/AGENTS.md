...
Admin order oversight: a status-grouped management board, a paginated/sortable listing, and status-transition management (which is also where the delivery person is assigned).
...
| Delivery person is assigned only on the shipped transition | `deliveryPersonId` is required when moving an order to `SHIPPED` and is never editable afterwards |
