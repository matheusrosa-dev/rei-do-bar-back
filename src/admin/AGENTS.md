...
The admin backoffice, protected by HTTP Basic Auth. Each managed resource (products, categories, customers, orders, inventory, settings, notifications) is its own NestJS sub-module that mirrors the standard module/controller/service/`dtos` layout (some also carry an event listener). Operations include catalog management, inventory movements and stock control, activation/deactivation, guarded deletion, order listing with status management, runtime settings management, and push notifications.
