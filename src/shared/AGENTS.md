...
Standalone functions (no classes) covering nine concerns: digest hashing and constant-time comparison, **password hashing**, one-time-code generation, **opaque-token generation**, timezone-aware dates (luxon, `America/Sao_Paulo`), the **recent-orders window**, **product totals**, **Prisma error predicates**, and the generic **exact-permutation check** (an id list against a set — same size, no duplicate, every id present) both catalog reorder endpoints validate their full-snapshot payloads with.
